	...
	<div class="task-detail-info">
				...
				if ($arResult["LIKE_TEMPLATE"] == 'like_react')
				{
					?><div class="task-detail-like"><?
						...
						?><span id="bx-ilike-button-<?=htmlspecialcharsbx($voteId)?>" class="feed-inform-ilike feed-new-like"><?
							?><span class="bx-ilike-left-wrap<?=(isset($templateData["RATING"]) && isset($templateData["RATING"]["USER_HAS_VOTED"]) && $templateData["RATING"]["USER_HAS_VOTED"] == "Y" ? ' bx-you-like-button' : '')?>"><a href="#like" class="bx-ilike-text"><?=Loc::getMessage('RATING_LIKE_EMOTION_'.$emotion)?></a></span><?
						?></span><?
					?></div><?
		...
		<? if ($templateData["SHOW_USER_FIELDS"]): ?>
			<div class="task-detail-properties">
				...
				{
					if (
						(empty($userField["VALUE"]) && $userField["VALUE"] !== '0') ||
...
<script>
	...
	new BX.Tasks.Component.TaskView({
		messages: {
			...
			TASKS_STATUS_8: "<?=CUtil::JSEscape(Loc::getMessage("TASKS_STATUS_8"))?>", //Новый статус ASSESSMENT
